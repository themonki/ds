...
public class GuiCatalogarModificar extends JFrame {
	...
	private JPanel panel, panel2, panel3, panel4, panel5, panelConAutores,
			panelConpalabrasC, panel8, panelConAreas;
	private JLabel tipoMaterial, numeroIndentificacion, tituloPrincipal,
			idioma, autor, tituloSecundario, traducido, editorial,
			derechosAutor, descripcion, indicacion, palabrasClave,
			fechaPublicacion, areas;
	...
	private JTextField campoEditorial, campoNumeroIdentificacion,
			campoTituloSecundario, campoTituloPpal;

	private JButton botonCatalogar, nuevaArea, nuevotipo, nuevoAutor,
			nuevoidioma, nuevapalabra;
	// faltan las fechas /////////****************///
	...
	public void initComponents() {
		...
		JLabel autores = new JLabel(
				"           Lista De Autores Actual.           ", JLabel.CENTER), palabrasC = new JLabel(
				"         Palabras Clave Actuales.          "), areaPertenece = new JLabel(
				"          Areas Actuales.              ");
		...
		panelConpalabrasC.setBorder(BorderFactory
		...
		GridBagConstraints restriccionCampo = new GridBagConstraints(), restriccionEtiquetas = new GridBagConstraints(), restriccionBotones = new GridBagConstraints();
		...
		restriccionEtiquetas.insets = new Insets(0, 14, 0, 0);// espacios entre
																// componentes
	 ...
	 * 
	 * GuiCatalogarModificar ventana; ventana = new GuiCatalogarModificar();
	...
	private class ManejadorBoton implements ActionListener {
		...
		public void actionPerformed(ActionEvent event) {
			...
			// doc = new Documento(null, campoIdioma.getSelectedItem(),
			// campoDerechosAutor.getSelectedItem(), campoDescripcion.getText(),
			// softwareRecomendado, resolucion, editorial, formato,
			// tituloPrincipal, tituloSecundario, null, , fechaPublicacion,
			// fechaCatalogacion, loginCatalogador,
			// campoTipoMaterial.getSelectedItem(), autores, areas,
			// palabrasClave)
	...
	private class ManejadorComboBox implements ActionListener {
		...
		public void actionPerformed(ActionEvent event) {
			...
			if (event.getSource() == campoAutor) {
				...
				if (autoresActualVector.indexOf(campoAutor.getSelectedItem()) == -1) {
					autoresActualVector.add((String) campoAutor
							.getSelectedItem());
...

			{
				...
				if (palabActualVec.indexOf(campoPalabras.getSelectedItem()) == -1) {
					palabActualVec
							.add((String) campoPalabras.getSelectedItem());
		...
		private class eventoMouse implements MouseListener {
			...
			int panel = 0;// usado para saber de que panel es la palabra autor
							// area o palabraclave
