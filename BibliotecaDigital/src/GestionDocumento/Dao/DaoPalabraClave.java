...
public class DaoPalabraClave
{
	...
	public PalabraClave consultarPalabraClave(String palabra)
	{
		...
		sqlSelect = "SELECT * FROM Palabra_Clave WHERE Palabra_Clave.nombre='" + palabra
				+ "'";
		try {
			Connection conn = this.fachada.conectar();
			Statement sentencia = conn.createStatement();
	...
	{
		PalabraClave a = new PalabraClave("permutacion","de cuantas maneras");
		DaoPalabraClave da = new DaoPalabraClave();
		System.out.println(da.guardarPalabraClave(a));
		da.consultarPalabraClave("permutacion");
		da.consultarPalabraClave("otra");
		da.consultarPalabras();
	}*/
}