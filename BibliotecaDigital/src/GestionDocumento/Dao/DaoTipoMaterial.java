...
public class DaoTipoMaterial
{
	...
	public TipoMaterial consultarTipoMaterial(String parametro)
	{
		...
		sqlSelect = "SELECT * FROM TipoMaterial WHERE TipoMaterial.tipo_nombre='" + parametro
				+ "'";
		try {
			Connection conn = this.fachada.conectar();
			Statement sentencia = conn.createStatement();
			...
			if(tabla.next()) {
				...
				/*probando*/
				System.out.println("Tipo Nombre: " + tabla.getString("tipo_nombre") +
	...
	{
		TipoMaterial a = new TipoMaterial("tesis","trabajo final");
		DaoTipoMaterial da = new DaoTipoMaterial();
		//System.out.print(da.guardarTipoMaterial(a));
		da.consultarTipoMaterial("tesis");
		da.consultarTodosTipoMaterial();
	}*/
}